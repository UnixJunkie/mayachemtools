...
def ProcessSaltBridgesChainResiduesOptionsInfo(SaltBridgesOptionName, SaltBridgesOptionValue):
    """Process specified salt bridges chain residues using command line option.

    Arguments:
        SaltBridgesOptionName (str): Name of command line  option.
        SaltBridgesOptionValue (str): Value for command line option.

    Returns:
        dict: A dictionary containing salt bridges residue types and residue
            names.

    Examples:

        SaltBridgesChainResiduesInfo =
            PyMOLUtil.ProcessSaltBridgesChainResiduesOptionsInfo(
            "--saltBridgesChainResidues", OptionsInfo["SaltBridgesChainResidues"])

    """
    
    # Set up  default values for salt bridges chain residues...
    SaltBridgesChainResiduesInfo = {"Positively_Charged": ["ARG", "LYS", "HIS", "HSP"], "Negatively_Charged": ["ASP", "GLU"]}
    
    SaltBridgesChainResidues = SaltBridgesOptionValue
    if re.match("^auto$", SaltBridgesChainResidues, re.I):
        return SaltBridgesChainResiduesInfo
    
    # Parse salt bridges chain residues types and residue names...
    SaltBridgesChainResiduesWords = SaltBridgesChainResidues.split(",")
    if len(SaltBridgesChainResiduesWords) % 2:
        MiscUtil.PrintError("The number of comma delimited salt bridges residue types and names, %d, specified using \"%s\" option must be a multple of 2." % (len(SaltBridgesChainResiduesWords), SaltBridgesOptionName))
    
    # Set up canonical salt bridges chain residue types...
    ValidSaltBridgesChainResidueTypes = []
    CanonicalSaltBridgesChainResiduesMap = {}
    for Type in sorted(SaltBridgesChainResiduesInfo):
        ValidSaltBridgesChainResidueTypes.append(Type)
        CanonicalSaltBridgesChainResiduesMap[Type.lower()] = Type
    
    # Validate and process specified values...
    for Index in range(0, len(SaltBridgesChainResiduesWords), 2):
        Type = SaltBridgesChainResiduesWords[Index].strip()
        Residue = SaltBridgesChainResiduesWords[Index + 1].strip()
        
        Residue = re.sub("[ ]+", " ", Residue)
        ResidueWords = Residue.split(" ")
        
        CanonicalType = Type.lower()
        if not CanonicalType in CanonicalSaltBridgesChainResiduesMap:
            MiscUtil.PrintError("The salt bridges chain residue type, %s, specified using \"%s\" option  is not a valid type: Supported salt bridges chain residue types: %s" % (Type, SaltBridgesOptionName, ", ".join(ValidSaltBridgesChainResidueTypes)))
        Type = CanonicalSaltBridgesChainResiduesMap[CanonicalType]
        
        if not Residue or len(ResidueWords) == 0:
            MiscUtil.PrintError("No residue specified for salt bridges chain residue type, %s, using \"%s\" option." % (Type, SaltBridgesOptionName))
        
        SaltBridgesChainResiduesInfo[Type] = ResidueWords
...
def SetupPMLForLigandView(Name, Selection, LigandResName, Enable = True, IgnoreHydrogens = False):
    ...
    Arguments:
        ...
        IgnoreHydrogens (bool): Ignore hydrogens.
    ...
    IgnoreHydrogensClause = " and (not hydro)" if IgnoreHydrogens else ""
    PMLCmds.append("""cmd.create("%s", "((%s) and organic and (resn %s)%s)")""" % (Name, Selection, LigandResName, IgnoreHydrogensClause))
...
def SetupPMLForLigandsSDFileView(Name, SDFile, Enable = True, IgnoreHydrogens = False):
    """Setup PML commands for creating a ligand view corresponding to ligands
    present in a SD file. The ligand is colored by atom types and displayed as 'sticks'.

    Arguments:
        Name (str): Name of a new PyMOL ligand(s) object.
        SDFile (str): Name of SD file.
        Enable (bool): Display status of ligand object.
        IgnoreHydrogens (bool): Ignore hydrogens.
    ...
    PMLCmds.append("""cmd.load("%s", "%s")""" % (SDFile, Name))
    
    PMLCmds.append("""cmd.hide("everything", "%s")""" % (Name))
    PMLCmds.append("""util.cbag("%s", _self = cmd)""" % (Name))
    PMLCmds.append("""cmd.show("sticks", "%s")""" % (Name))
    if IgnoreHydrogens:
        PMLCmds.append("""cmd.remove("(%s and hydro)")""" % (Name))
...
def SetupPMLForLigandPocketView(Name, Selection, LigandSelection, DistanceCutoff, Enable = True, IgnoreHydrogens = False):
    ...
    IgnoreHydrogensClause = " and (not hydro)" if IgnoreHydrogens else ""
    PMLCmds.append("""cmd.create("%s", "((byresidue (%s) within %.1f of (%s)) and (not solvent) and (not inorganic) and (not organic)%s)")""" % (Name, Selection, DistanceCutoff, LigandSelection, IgnoreHydrogensClause))
...
def SetupPMLForPolarContactsView(Name, Selection1, Selection2, Enable = True, Color = "yellow", Cutoff = None):
    ...
    PMLCmds.append(SetupPMLForDeepColoring(Name, Color))
    PMLCmds.append(SetupPMLForEnableDisable(Name, Enable))
...
def SetupPMLForDeepColoring(Name, Color):
    """Setup PML command for deep coloring based on PyMOL version number.

    Arguments:
        Name (str): Name of a PyMOL object.
        Color (str): Color for PyMOL object.

    Returns:
        str: PML command for deep coloring.

    """

    if cmd.get_version()[1] < 2.0:
        return("""util.color_deep("%s", "%s")""" % (Color, Name))
    else:
        return("""cmd.color_deep("%s", "%s")""" % (Color, Name))
...
def SetupPMLForSelectionDisplayView(Name, Selection, DisplayAs, Color = None, Enable = True, IgnoreHydrogens = False):
    ...
    PMLCmds = []
    if IgnoreHydrogens:
        PMLCmds.append("""cmd.create("%s", "((%s) and (not hydro))")""" % (Name, Selection))
    else:
        PMLCmds.append("""cmd.create("%s", "(%s)")""" % (Name, Selection))
    PMLCmds.append("""cmd.hide("everything", "%s")""" % (Name))
    PMLCmds.append("""cmd.show("%s", "%s")""" % (DisplayAs, Name))
...
def SetupPMLForDisulfideBondsView(Name, Selection, DisplayAs, Enable = True):
    """Setup PML commands for creating a  view corresponding to
    disulfide bonds present in a selection.

    Arguments:
        Name (str): Name of a new PyMOL disulfide bonds object.
        Selection (str): Name of PyMOL selection.
        DisplayAs (str): Any valid display type such as lines, sticks, ribbon,
            cartoon, or surface
        Enable (bool): Display status of disulfide bonds object.

    Returns:
        str: PML commands for disulfide bonds view.

    """
    
    PMLCmds = []
    
    DisulfideBondsSelection = "(byres (((%s) and (resn CYS+CYX) and (name SG)) and bound_to ((%s) and (resn CYS+CYX) and (name SG))))" % (Selection, Selection)
    PMLCmds.append("""cmd.create("%s", "(%s)")""" % (Name, DisulfideBondsSelection))
    PMLCmds.append("""cmd.hide("everything", "%s")""" % (Name))
    PMLCmds.append("""util.cbag("%s", _self = cmd)""" % (Name))
    PMLCmds.append("""cmd.show("%s", "%s")""" % (DisplayAs, Name))
    PMLCmds.append(SetupPMLForEnableDisable(Name, Enable))
...
def SetupPMLForSaltBridgesResiduesView(Name, Selection, Residues, DisplayAs, Enable = True):
    """Setup PML commands for creating a  view corresponding to residues
    for salt bridges present in a selection.
    ...
    Arguments:
        ...
        DisplayAs (str): Any valid display type such as lines, sticks, ribbon,
            cartoon, or surface
        Enable (bool): Display status of salt bridges residues object.

    Returns:
        str: PML commands for salt bridges residues view.

    """
    
    PMLCmds = []
    
    SaltBridgesResiduesSelection = "((%s) and (resn %s) and (not name N+O) and (not hydro))" % (Selection, "+".join(Residues) )
    
    PMLCmds.append("""cmd.create("%s", "(%s)")""" % (Name, SaltBridgesResiduesSelection))
    PMLCmds.append("""cmd.hide("everything", "%s")""" % (Name))
    PMLCmds.append("""util.cbag("%s", _self = cmd)""" % (Name))
    PMLCmds.append("""cmd.show("%s", "%s")""" % (DisplayAs, Name))
    PMLCmds.append(SetupPMLForEnableDisable(Name, Enable))
