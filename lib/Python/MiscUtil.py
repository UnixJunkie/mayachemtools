...
def ProcessOptionOutfileParameters(ParamsOptionName, ParamsOptionValue, InfileName = None, OutfileName = None, ParamsDefaultInfo = None):
    ...
    ParamsInfo = {'Compute2DCoords': 'auto', 'Kekulize': True, 'SMILESKekulize': False, 'SMILESDelimiter': ' ', 'SMILESIsomeric': True,  'SMILESTitleLine': True, 'SMILESMolName': True, 'SMILESMolProps': False}
...
def _ProcessInfileAndOutfileParameters(Mode, ParamsInfo, ParamsOptionName, ParamsOptionValue, InfileName, OutfileName):
    ...
    for Index in range(0, len(ParamsOptionValueWords), 2):
        ...
        if re.match("^(Sanitize|StrictParsing|RemoveHydrogens|Kekulize|SMILESKekulize|SMILESIsomeric)$", ParamName, re.I):
            if not re.match("^(Yes|No|True|False)$", Value, re.I):
                PrintError("The parameter value, %s, specified for parameter name, %s, using \"%s\" option is not a valid value. Supported values: Yes No True False" % (Value, Name, ParamsOptionName))
            ParamValue = True
            if re.match("^(No|False)$", Value, re.I):
