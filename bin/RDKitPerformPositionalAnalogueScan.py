        ...
        for AtomIndicesCombination in combinations(MolMatchedAtomIndices, OptionsInfo["ComboSearchAtoms"]):
            ...
            for AtomIndex in AtomIndicesCombination:
                if OptionsInfo["AttachAtomsMode"]:
                    ...
                    AttachAtomIndex = MolAnalogue.GetNumAtoms() - 1 + TargetPatternInfo["AttachSMILESAtomIndexOffsets"][TargetPatternIndex]
            ...
            for PatternInfo in TargetPatternTandemScanInfo:
                ...
                for NameIndex, Name in enumerate(PatternInfo["Names"]):
                    ...
                    if re.match("^AttachAtoms$", Name, re.I):
                        MolAnalogue = AttachAtom(MolAnalogue, AtomIndex, PatternObject, BondOrder = Chem.rdchem.BondType.SINGLE)
                        ...
                        AttachAtomIndex = MolAnalogue.GetNumAtoms() - 1 + PatternInfo["AttachSMILESAtomIndexOffsets"][NameIndex]
                        MolAnalogue = AttachMol(MolAnalogue, AtomIndex, PatternObject, AttachAtomIndex, BondOrder = Chem.rdchem.BondType.SINGLE)
    ...
    if re.match("^(AttachAtoms|ReplaceAtoms)$", Mode, re.I):
        ...
        for ElementSymbol in TargetPatternInfo["Patterns"]:
            if not RDKitUtil.IsValidElementSymbol(ElementSymbol):
                MiscUtil.PrintError("The target pattern, \"%s\", specified using \"-t, --targetPattern\" must be a valid atomic symbol during, %s, value of \"-m, --mode\" option." % (ElementSymbol, Mode))
    elif re.match("^AttachSMILES$", Mode, re.I):
        TargetPatternInfo = InitializeTargetPatternInfo(TargetPattern)
        ...
        for RxnPattern in TargetPatternInfo["Patterns"]:
            try:
                Rxn = AllChem.ReactionFromSmarts(RxnPattern)
            except Exception as ErrMsg:
        ...
        if re.match("^(AttachAtoms|ReplaceAtoms)$", OperationName, re.I):
            for Pattern in OperationPatterns:
                if not RDKitUtil.IsValidElementSymbol(Pattern):
                    MiscUtil.PrintError("The target pattern, \"%s\", specified using \"-t, --targetPattern\" must be a valid atomic symbol for operation, %s, during, %s, value of \"-m, --mode\" option." % (Pattern, OperationName, Mode))
