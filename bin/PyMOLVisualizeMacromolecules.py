...
def WriteChainLigandView(OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID):
    ...
    for GroupID in ["Ligand", "Pocket", "PocketSolvent", "PocketInorganic"]:
        ...
        if re.match("^Ligand$", GroupID, re.I):
            ...
            PML = PyMOLUtil.SetupPMLForLigandView(GroupTypeObjectName, ComplexName, LigandID, Enable = True, IgnoreHydrogens = OptionsInfo["IgnoreHydrogens"])
            OutFH.write("%s\n" % PML)
        elif re.match("^Pocket$", GroupID, re.I):
            OutFH.write("""\n""\n"Setting up views for pocket around ligand %s in chain %s..."\n""\n""" % (LigandID, ChainID))
            PML = PyMOLUtil.SetupPMLForLigandPocketView(GroupTypeObjectName, ComplexName, LigandName, OptionsInfo["PocketDistanceCutoff"], Enable = True, IgnoreHydrogens = OptionsInfo["IgnoreHydrogens"])
            OutFH.write("%s\n" % PML)
...
def WriteChainDockedPosesView(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    """Write out PML for viewing docked poses for SD files in a chain."""

    if not GetChainAloneDockedPosesStatus(FileIndex, ChainID):
        return
    
    SpecifiedChainsAndLigandsInfo = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]
    for SDFileIndex, SDFile in enumerate(SpecifiedChainsAndLigandsInfo["DockedPosesSDFiles"][ChainID]):
        WriteChainDockedPosesViewForSDFile(OutFH, FileIndex, PyMOLObjectNames, ChainID, SDFileIndex)

    GenerateAndWritePMLForGroup(OutFH, PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroup"], PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroupMembers"], False, "close")
    
def WriteChainDockedPosesViewForSDFile(OutFH, PDBFileIndex, PyMOLObjectNames, ChainID, SDFileIndex):
    """Write out PML for viewing docked poses for a SD file in a chain."""

    DockedPosesInfo = OptionsInfo["DockedPosesInfo"]
    
    LigandID = DockedPosesInfo["LigandID"][PDBFileIndex]
    UseSDFileAsLigandID = DockedPosesInfo["UseSDFileAsLigandID"][PDBFileIndex]
    SDFile = DockedPosesInfo["SDFiles"][PDBFileIndex][SDFileIndex]
    SDFileID = DockedPosesInfo["SDFilesIDs"][PDBFileIndex][SDFileIndex]

    OutFH.write("""\n""\n"Setting up views for docked poses in SD file ID %s for chain %s..."\n""\n""" % (SDFileID, ChainID))
    
    # Setup poses view...
    PosesName = PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["Poses"]
    PML = PyMOLUtil.SetupPMLForLigandsSDFileView(PosesName, SDFile, Enable = True, IgnoreHydrogens = OptionsInfo["IgnoreHydrogens"])
    OutFH.write("%s\n" % PML)
    
    # Setup pocket view...
    PocketName = PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["Pocket"]
    ComplexName = PyMOLObjectNames["Chains"][ChainID]["ChainComplex"]
    LigandName = PosesName if UseSDFileAsLigandID else PyMOLObjectNames["Ligands"][ChainID][LigandID]["Ligand"]
    PML = PyMOLUtil.SetupPMLForLigandPocketView(PocketName, ComplexName, LigandName, OptionsInfo["PocketDistanceCutoff"], Enable = True, IgnoreHydrogens = OptionsInfo["IgnoreHydrogens"])
    OutFH.write("%s\n" % PML)
    OutFH.write("""cmd.set("label_color", "%s", "%s")\n""" % (OptionsInfo["PocketLabelColor"], PocketName))
    
    # Setp polar contacts view...
    PolarContactsName = PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["PolarContacts"]
    PolarContactsColor = OptionsInfo["PocketContactsLigandColor"]
    PML = PyMOLUtil.SetupPMLForPolarContactsView(PolarContactsName, PosesName, PocketName, Enable = False, Color = PolarContactsColor, Cutoff = OptionsInfo["PocketContactsCutoff"])
    OutFH.write("\n%s\n" % PML)
    OutFH.write("""cmd.set("label_color", "%s", "%s")\n""" % (PolarContactsColor, PolarContactsName))
    
    # Hydrophobic contacts view...
    HydrophobicContactsName = PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["HydrophobicContacts"]
    HydrophobicContactsColor = OptionsInfo["PocketContactsLigandHydrophobicColor"]
    PML = PyMOLUtil.SetupPMLForHydrophobicContactsView(HydrophobicContactsName, PosesName, PocketName, Enable = False, Color = HydrophobicContactsColor, Cutoff = OptionsInfo["PocketContactsCutoff"])
    OutFH.write("\n%s\n" % PML)
    OutFH.write("""cmd.set("label_color", "%s", "%s")\n""" % (HydrophobicContactsColor, HydrophobicContactsName))
    
    # Setup docked poses group for a SD file...
    Action = "open" if SDFileIndex == 0 else "close"
    Enable = True if SDFileIndex == 0 else False
    GenerateAndWritePMLForGroup(OutFH, PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["DockedPosesGroupName"], PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["DockedPosesGroupMembers"], Enable, Action)
...
def WriteChainAloneChainSelectionsView(OutFH,  FileIndex, PyMOLObjectNames, ChainID):
    ...
    for Index in range(0, len(OptionsInfo["ChainSelectionsInfo"]["Names"])):
        ...
        PML = PyMOLUtil.SetupPMLForSelectionDisplayView(SelectionObjectName, SelectionCmd, OptionsInfo["SelectionsChainStyle"], Enable = True, IgnoreHydrogens = OptionsInfo["IgnoreHydrogens"])
        OutFH.write("\n%s\n" % PML)
...
def WriteChainAloneDisulfideBondsView(OutFH,  FileIndex, PyMOLObjectNames, ChainID):
    ...
    GenerateAndWritePMLForGroup(OutFH, PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroup"], PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroupMembers"], True, "close")
...
def WriteChainAloneSaltBridgesView(OutFH,  FileIndex, PyMOLObjectNames, ChainID):
    ...
    PositivelyChargedResiduesName = PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesPositivelyCharged"]
    PML = PyMOLUtil.SetupPMLForSaltBridgesResiduesView(PositivelyChargedResiduesName, ChainName, OptionsInfo["SaltBridgesChainResiduesInfo"]["Positively_Charged"], "lines", Enable = True)
    OutFH.write("\n%s\n" % PML)
    
    NegativelyChargedResiduesName = PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesNegativelyCharged"]
    PML = PyMOLUtil.SetupPMLForSaltBridgesResiduesView(NegativelyChargedResiduesName, ChainName, OptionsInfo["SaltBridgesChainResiduesInfo"]["Negatively_Charged"], "lines", Enable = True)
    OutFH.write("\n%s\n" % PML)
    
    GenerateAndWritePMLForGroup(OutFH, PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesGroup"], PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesGroupMembers"], True, "open")
    ...
    PML = PyMOLUtil.SetupPMLForPolarContactsView(ContactsName, PositivelyChargedResiduesName, NegativelyChargedResiduesName, Enable = True, Color = ContactsColor, Cutoff = OptionsInfo["SaltBridgesChainCutoff"])
    OutFH.write("\n%s\n" % PML)
    
    OutFH.write("""cmd.set("label_color", "%s", "%s")\n""" % (ContactsColor, ContactsName))
    
    # Setup salt bridges group...
    GenerateAndWritePMLForGroup(OutFH, PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroup"], PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroupMembers"], False, "close")
...
def WriteResidueTypesResiduesAndSurfaceView(OutFH, SelectionObjectName, Name, SurfaceName, ResiduesColor, ResiduesNames, NegateResidueNames):
    ...
    # Setup residues...
    PML = PyMOLUtil.SetupPMLForSelectionDisplayView(Name, Selection, "lines", ResiduesColor, True, IgnoreHydrogens = OptionsInfo["IgnoreHydrogens"])
    OutFH.write("\n%s\n" % PML)

    # Setup surface...
    PML = PyMOLUtil.SetupPMLForSelectionDisplayView(SurfaceName, Selection, "surface", ResiduesColor, True, IgnoreHydrogens = OptionsInfo["IgnoreHydrogens"])
    OutFH.write("\n%s\n" % PML)
...
def DeleteEmptyChainDisulfideBondsObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    WritePMLToCheckAndDeleteEmptyObjects(OutFH, PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsResidues"], PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroup"])
...
def DeleteEmptyChainSaltBridgesObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    WritePMLToCheckAndDeleteEmptyObjects(OutFH, ",".join(Names), PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroup"])
...
def SetupPyMOLObjectNames(FileIndex):
    ...
    PyMOLObjectNames["DockedPosesSDFile"] = {}
...
def SetupPyMOLObjectNamesForDockedPoses(FileIndex, PyMOLObjectNames, ChainID):
    """Stetup groups and objects for docked poses in SD files for a chain."""

    PyMOLObjectNames["DockedPosesSDFile"][ChainID] = {}
    ...
    if "DockedPosesGroup" not in PyMOLObjectNames["Chains"][ChainID]:
        ...
        ChainGroupName = PyMOLObjectNames["Chains"][ChainID]["ChainGroup"]
        DockedPosesGroupName = "%s.Docked_Poses" % (ChainGroupName)
    ...
    SpecifiedChainsAndLigandsInfo = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]
    for SDFileIndex, SDFile in enumerate(SpecifiedChainsAndLigandsInfo["DockedPosesSDFiles"][ChainID]):
        SetupPyMOLObjectNamesForDockedPosesSDFile(FileIndex, PyMOLObjectNames, ChainID, SDFileIndex)
    
def SetupPyMOLObjectNamesForDockedPosesSDFile(PDBFileIndex, PyMOLObjectNames, ChainID, SDFileIndex):
    """Stetup groups and objects for docked poses in a SD file for a chain."""
    
    DockedPosesInfo = OptionsInfo["DockedPosesInfo"]
    
    SDFileID = DockedPosesInfo["SDFilesIDs"][PDBFileIndex][SDFileIndex]
    
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID] = {}
    
    DockedPosesGroupName = PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroup"]

    # Setup a docked poses level SDFile group...
    DockedPosesSDFileGroupName = "%s.%s" % (DockedPosesGroupName, SDFileID)
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["DockedPosesGroupName"] = DockedPosesSDFileGroupName
    PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroupMembers"].append(DockedPosesSDFileGroupName)

    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["DockedPosesGroupMembers"] = []

    # Setup objects for docked poses...
    PosesName = "%s.Poses" % DockedPosesSDFileGroupName
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["Poses"] = PosesName
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["DockedPosesGroupMembers"].append(PosesName)

    PocketName = "%s.Pocket" % DockedPosesSDFileGroupName
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["Pocket"] = PocketName
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["DockedPosesGroupMembers"].append(PocketName)
    
    PolarContactsName = "%s.Polar_Contacts" % DockedPosesSDFileGroupName
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["PolarContacts"] = PolarContactsName
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["DockedPosesGroupMembers"].append(PolarContactsName)
    
    HydrophobicContactsName = "%s.Hydrophobic_Contacts" % DockedPosesSDFileGroupName
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["HydrophobicContacts"] = HydrophobicContactsName
    PyMOLObjectNames["DockedPosesSDFile"][ChainID][SDFileID]["DockedPosesGroupMembers"].append(HydrophobicContactsName)
...
def ProcessResidueTypesAndSurfaceAndChainSelectionsOptions(FileIndex, SpecifiedChainsAndLigandsInfo):
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        ...
        # Process disulfide bonds fo chains...
        ...
        # Process salt bridges bonds fo chains...
...
def ProcessDockedPosesInfoForInfileAndChains(FileIndex, SpecifiedChainsAndLigandsInfo, DockedPosesInfo):
    ...
    SpecifiedChainsAndLigandsInfo["DockedPosesSDFiles"] = {}
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        ...
        SpecifiedChainsAndLigandsInfo["DockedPosesSDFiles"][ChainID] = []
        if DockedPoses:
            SpecifiedChainsAndLigandsInfo["DockedPosesSDFiles"][ChainID].extend(DockedPosesInfo['SDFiles'][FileIndex])
...
def ProcessDockedPosesOptionsInfo(DockedPosesOptionName, DockedPosesOptionValue):
    ...
    DockedPosesInfo["UseSDFileAsLigandID"] = {}
    DockedPosesInfo["SDFiles"] = {}
    DockedPosesInfo["SDFilesRoots"] = {}
    DockedPosesInfo["SDFilesIDs"] = {}
    
    # Parse docked poses values...
    DockedPosesWords = DockedPoses.split(",")
    if len(DockedPosesWords) % 2:
        MiscUtil.PrintError("The number of comma delimited docked poses values, %d, specified using \"%s\" option must be a multple of 4." % (len(DockedPosesWords), DockedPosesOptionName))
    ...
    for Index in range(0, len(DockedPosesWords), 4):
        ...
        SDFiles = DockedPosesWords[Index + 3].strip()
        
        SDFiles = re.sub("[ ]+", " ", SDFiles)
        SDFiles = SDFiles.split(" ")
        ...
        if PDBFileIndex is None:
            MiscUtil.PrintError("The PDB file specified, %s, using option \"--dockedPoses\" is not valid. It must be specified as an input file." % (PDBFile))

        if PDBFileIndex in DockedPosesInfo['PDBFileIndices']:
            MiscUtil.PrintError("The PDB file specified, %s, using option \"--dockedPoses\" is not valid. It is a duplicate and has already been specified." % (PDBFile))
        ...
        if ChainID not in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
            MiscUtil.PrintError("The chain ID, %s, specified for PDB file, %s, using option \"--dockedPoses\" is not valid. It must be present in valid chain IDs corresponding to option \"-c, --chainIDs\" ." % (ChainID, PDBFile))

        # Process ligand ID...
        UseSDFileAsLigandID = True if re.match("^UseSDFile$", LigandID, re.I) else False
        if not UseSDFileAsLigandID:
            if LigandID not in SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID]:
                MiscUtil.PrintError("The ligand ID, %s, specified for chain ID, %s, and PDB file, %s, using option \"--dockedPoses\" is not valid. It must be present in valid ligand IDs corresponding to option \"-l, --ligandIDs\" ." % (LigandID, ChainID, PDBFile))

        # Process SD files...
        SDFilesRoots = []
        SDFilesIDs = []
        for SDFile in SDFiles:
            MiscUtil.ValidateOptionFilePath("--dockedPoses", SDFile)
            MiscUtil.ValidateOptionFileExt("--dockedPoses", SDFile, "sdf sd")
            
            FileDir, FileName, FileExt = MiscUtil.ParseFileName(SDFile)
            SDFileRoot = FileName
            
            # Clean up SD file names for generating PyMOL object names...
            SDFileID = re.sub("[^a-zA-Z0-9]", "_", FileName)
            
            if SDFileRoot in SDFilesRoots:
                MiscUtil.PrintError("The SD file specified, %s, using option \"--dockedPoses\" is not valid. It is a duplicate and has already been specified for a PDB file." % (SDFile))
            
            SDFilesRoots.append(SDFileRoot)
            SDFilesIDs.append(SDFileID)
        ...
        DockedPosesInfo["UseSDFileAsLigandID"][PDBFileIndex] = UseSDFileAsLigandID
        
        DockedPosesInfo["SDFiles"][PDBFileIndex] = SDFiles
        DockedPosesInfo["SDFilesRoots"][PDBFileIndex] = SDFilesRoots
        DockedPosesInfo["SDFilesIDs"][PDBFileIndex] = SDFilesIDs
...
def ProcessSaltBridgesChainResidues():
    ...
    SaltBridgesChainResiduesInfo = PyMOLUtil.ProcessSaltBridgesChainResiduesOptionsInfo("--saltBridgesChainResidues", OptionsInfo["SaltBridgesChainResidues"])
...
Usage:
    PyMOLVisualizeMacromolecules.py [--align <yes or no>] [--alignMethod <align, cealign, super>]
                                    ...
                                    [--allowEmptyObjects <yes or no>] [--chainIDs <First, All or ID1,ID2...>] [--disulfideBondsChain <yes or no>]
                                    [--dockedPoses <PDBFile,ChainID,LigandID,SDFiles,...>]
...
Options:
    ...
    --dockedPoses <PDBFile,ChainID,LigandID,SDFiles,...>  [default: none]
        PDB file name, pocket chain ID, ligand specification, and SD files to use
        for creating pockets to visualize docked poses.
        
        It's a quartet of comma limited values corresponding to PDF file name,
        pocket chain ID, ligand ID, and SD files. Multiple SD file names are delimited
        by space.
        ...
        SD files by specifying 'UseSDFile' 'to select residues in 'ChainID' for creating
...
Examples:
    ...
    ligand and a chain, along with visualization of other information, in a PDB file,
    and generate a PML file, type:
