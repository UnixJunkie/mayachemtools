...
def GenerateCubeFilesUsingCubeprop(Psi4Handle, WaveFunction, Mol, MolNum, CubeFilesDir, CubeFilesDirPath):
    ...
    try:
        Psi4Handle.set_options({"CUBEPROP_TASKS": ['DUAL_DESCRIPTOR'],
...
def VisualizeDualDescriptors():
    ...
    for Mol in Mols:
        ...
        GenerateAndWritePMLForGroup(OutFH, PyMOLObjectNames["DualGroup"], PyMOLObjectNames["DualGroupMembers"], Enable, Action)
...
def WriteDualDescriptorView(OutFH, CubeFilesInfo, PyMOLObjectNames):
    ...
    if CubeFilesInfo["SetupLocalVolumeColorRamp"]:
        GenerateAndWritePMLForVolumeColorRamp(OutFH, CubeFilesInfo["VolumeColorRampName"], ContourLevel1, ContourColor1, ContourLevel2, ContourColor2, ContourWindowFactor, ContourColorOpacity)
    
    # Volume...
    DualVolumeName = PyMOLObjectNames["DualVolume"]
    PMLCmds = []
    PMLCmds.append("""cmd.volume("%s", "%s", "%s")""" % (DualVolumeName, DualCubeName, CubeFilesInfo["VolumeColorRampName"]))
    ...
    GenerateAndWritePMLForGroup(OutFH, PyMOLObjectNames["DualMeshGroup"], PyMOLObjectNames["DualMeshGroupMembers"], False, "open")
    ...
    GenerateAndWritePMLForGroup(OutFH, PyMOLObjectNames["DualSurfaceGroup"], PyMOLObjectNames["DualSurfaceGroupMembers"], True, "open")
...
def RetrieveMolCubeFilesInfo(Mol, MolNum):
    ...
    if  len(CubeFiles) > 1:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning("Multiple cube files available for molecule %s; Using first cube file, %s, to visualize dual descriptor..." % (RDKitUtil.GetMolName(Mol, MolNum), CubeFileName))
    CubeFilesInfo["FileName"] = CubeFileName
    
    IsocontourRangeMin, IsocontourRangeMax, ContourLevel1, ContourLevel2 = SetupIsocontourRangeAndContourLevels(CubeFileName)
    ...
    CubeFilesInfo["SetupLocalVolumeColorRamp"] = False if OptionsInfo["PyMOLViewParams"]["SetupGlobalVolumeColorRamp"] else True
...
def CheckAndValidateMolecule(Mol, MolCount = None):
    ...
    if RDKitUtil.GetSpinMultiplicity(Mol) > 1:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning("Generation of dual descriptors not supported for open-shell systems. Ignoring  molecule containing unpaired electrons: %s\n" % MolName)
        return False
