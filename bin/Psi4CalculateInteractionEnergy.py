    ...
    for Mol in Mols:
        ...
        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning("Failed to calculate interaction energy for molecule %s" % RDKitUtil.GetMolName(Mol, MolCount))
...
def WorkerProcess(EncodedMolInfo):
    ...
    return [MolIndex, RDKitUtil.MolToBase64EncodedMolString(Mol, PropertyPickleFlags = Chem.PropertyPickleOptions.MolProps | Chem.PropertyPickleOptions.PrivateProps), CalcStatus, Energy, EnergyDetails]
...
def CalculateMoleculeInteractionEnergy(Psi4Handle, Psi4Mol, Mol, MolNum = None):
    ...
    try:
        if OptionsInfo["BSSEType"] is None:
            ...
            Energy = Psi4Handle.energy(MethodAndBasisSet, bsse_type = OptionsInfo["BSSEType"], molecule = Psi4Mol, return_wfn = False)
        Status = True
    ...
    if Status and OptionsInfo["SAPTMode"]:
        ...
        for EnergyType in EnergyTypeMap:
            try:
                ...
                if not OptionsInfo["QuietMode"]:
                    MiscUtil.PrintWarning("Failed to retrieve value for interaction energy components %s:\n%s\n" % (EnergyType, ErrMsg))
...
def CheckAndValidateMolecule(Mol, MolCount = None):
    ...
    if OptionsInfo["SAPTMode"] or OptionsInfo["SNSMP2Mode"]:
        if NumFragments != 2 :
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning("Ignoring molecule containing invalid number of fragments: %s. It must contain exaclty 2 fragments for calculation of interaction energy using SAPT or SNS-MP2 methodology.\n" % NumFragments)
            return False
    else:
        if NumFragments == 1:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning("Ignoring molecule containing invalid number of fragments: %s. It must contain more than 1 fragment for calculation of interaction energy using counterpoise correctio methodology.\n" % NumFragments)
            return False
...
def ProcessMethodNameAndBasisSetOptions():
    ...
    if BasisSetAuto:
        if SAPTMode:
            ...
            MiscUtil.PrintError("A basis set must be explicitly specified using \"-b, --basisSet\" option for, \"%s\", value of \"-m, --methodName\". " % (MethodName))

    if SNSMP2Mode and not MiscUtil.IsEmpty(BasisSet):
        MiscUtil.PrintError("The basis set value, \"%s\", specified using \"-b, --basisSet\" option is not valid. It must be an empty string SNS-MP2 calculations." % (Options["--basisSet"]))
...
def ProcessBSSETypeOption():
    ...
    BSSEType = None if re.match("^None$", Options["--bsseType"], re.I) or MiscUtil.IsEmpty(Options["--bsseType"]) else Options["--bsseType"]
    ...
    if BSSETypeAuto:
        ...
        if OptionsInfo["SAPTMode"] or OptionsInfo["SNSMP2Mode"]:
            if BSSEType is not None:
                MiscUtil.PrintError("The BSSE type value, \"%s\", specified using \"--bsseType\" option is not valid for SAPT or SNS-MP2 calculations. It must be set to None." % (Options["--bsseType"]))
        else:
            if BSSEType is None:
                MiscUtil.PrintWarning("The BSSE type value, \"%s\", specified using \"--bsseType\" option is not valid for \"%s\" calculations. Possible values: CP, noCP or VMFC" % (Options["--bsseType"], OptionsInfo["MethodName"]))
...
def ProcessSAPTEnergyDataFieldLabelsOption():
    ...
    ParamsIDsToTypes = {"ElectrostaticEnergy": "SAPT ELST ENERGY", "ExchangeEnergy": "SAPT EXCH ENERGY", "InductionEnergy": "SAPT IND ENERGY", "DispersionEnergy": "SAPT DISP ENERGY"}
    
    UnitsLabel = "(%s)" % Options["--energyUnits"]
    ParamsIDsToLabels = {"ElectrostaticEnergy": "Psi4_SAPT_Electrostatic_Energy %s" % UnitsLabel, "ExchangeEnergy": "Psi4_SAPT_Exchange_Energy %s" % UnitsLabel, "InductionEnergy": "Psi4_SAPT_Induction_Energy %s" % UnitsLabel, "DispersionEnergy": "Psi4_SAPT_Dispersion_Energy %s" % UnitsLabel}
