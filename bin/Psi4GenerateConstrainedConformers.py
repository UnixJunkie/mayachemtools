...
def ProcessMoleculesUsingSingleProcess(RefMol, Mols, Writer):
    ...
    for Mol in Mols:
        ...
        ConfMols, CalcStatus, ConfIDs, ConfEnergyValues, ConfScaffoldEmbedRMSDValues = GenerateMolConformers(Psi4Handle, Mol, RefMolCore, MolCount)
...
def WorkerProcess(EncodedMolInfo):
    ...
    # Generate conformers...
    ConfMols, CalcStatus, ConfIDs, ConfEnergyValues, ConfScaffoldEmbedRMSDValues = GenerateMolConformers(OptionsInfo["psi4"], Mol, RefMolCore, MolNum)
...
def ProcessMoleculesUsingMultipleProcessesAtConformersLevel(RefMol, Mols, Writer):
    ...
    for Mol in Mols:
        ...
        ConfMols, CalcStatus, ConfIDs, ConfEnergyValues, ConfScaffoldEmbedRMSDValues = ProcessConformersUsingMultipleProcesses(Mol, RefMolCore, MolCount)
...
def ProcessConformersUsingMultipleProcesses(Mol, RefMolCore, MolNum):
    ...
    # Filter conformers...
    SelectedMolConfs, SelectedMolConfIDs, SelectedConfEnergies, SelectedConfScaffoldEmbedRMSDValues = FilterMolConformers(Mol, MolNum, MolConfs, MolConfIDs, CalcEnergyMap, ScaffoldEmbedRMSDMap)
...
def ConformerWorkerProcess(EncodedMolInfo):
    ...
    return [MolConfNum, RDKitUtil.MolToBase64EncodedMolString(MolConf, PropertyPickleFlags = Chem.PropertyPickleOptions.MolProps | Chem.PropertyPickleOptions.PrivateProps), CalcStatus, Energy, ScaffoldEmbedRMSD]
...
def ProcessOptions():
    ...
    OptionsInfo["EnergyRMSDCalcModeBest"] = True if re.match("^BestRMSD$", Options["--energyRMSDCalcMode"], re.I) else False
    
    OptionsInfo["EnergyRMSDCutoff"] = float(Options["--energyRMSDCutoff"])
    
    OptionsInfo["EnergyRMSDCutoffMode"] = Options["--energyRMSDCutoffMode"]
    OptionsInfo["EnergyRMSDCutoffModeLowest"] = True if re.match("^Lowest$", Options["--energyRMSDCutoffMode"], re.I) else False
    ...
    if re.match("^auto$", EnergyWindow, re.I):
        ...
        if re.match("^kcal\/mol$", EnergyUnits, re.I):
            ...
            MiscUtil.PrintError("Failed to set default value for \"--energyWindow\". The value, %s, specified for option \"--energyUnits\" is not valid. " % EnergyUnits)
    else:
...
Examples:
    ...
    generating up to 50 conformations using ETKDG methodology followed by initial
